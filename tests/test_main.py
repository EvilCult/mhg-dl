...
def download_args():
    """下载命令的参数对象"""
    return Namespace(cid="12345", type="all", skip=None, pick=None)
...
def test_download_command_success(
    temp_dir, download_args, mock_manga_info, monkeypatch
):
    """测试完整的下载流程（正常情况）"""
    # Mock 所有外部依赖
    ...
    mock_get_chapter_image_urls = mock.Mock(
        return_value=["http://example.com/1.jpg", "http://example.com/2.jpg"]
    )
    ...
    monkeypatch.setattr(
        "mhg_dl.main.get_chapter_image_urls", mock_get_chapter_image_urls
    )
    ...
    mock_filter_chapter.assert_called_once_with(
        mock_manga_info.chapters, "all", None, None
    )
...
def test_download_command_without_cover(
    temp_dir, download_args, mock_manga_info, monkeypatch
):
    """测试没有封面时的处理"""
...
def test_download_command_creates_correct_directory_structure(
    temp_dir, download_args, monkeypatch
):
    """测试目录创建逻辑（有作者 vs 无作者）"""
    # 测试有作者的情况
...
def test_download_command_respects_chapter_filter(temp_dir, monkeypatch):
    ...
    # 创建带过滤的参数
    filtered_args = Namespace(cid="12345", type="单行本", skip="第1话", pick=None)
...
def test_download_command_with_empty_chapters(temp_dir, download_args, monkeypatch):
    ...
    empty_manga = MangaInfo(
        cid="12345", title="测试漫画", author="测试作者", chapters={}
    )
