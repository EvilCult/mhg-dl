...
def search_manga(query: str) -> list[MangaInfo]:
    ...
    while page < 4:
        search_url = SEARCH_URL.format(
            query=query, page_suffix="" if page == 1 else f"_p{page}"
        )
...
def fmt_manga_info(soup: BeautifulSoup) -> MangaInfo:
    ...
    for item in soup.select(".book-result > ul > li"):
        manga: MangaInfo = MangaInfo(
            ...
            author=item.select_one(".book-detail > dl > dd:nth-child(4)")
            .text.strip()
            .split("：")[1],
            year=item.select_one(
                ".book-detail > dl > dd:nth-child(3) > span > a"
            ).text.strip(),
            stat=item.select_one(
                ".book-detail > dl > dd:nth-child(2) > span > span"
            ).text.strip(),
        )
